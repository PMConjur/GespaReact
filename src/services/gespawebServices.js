...
//EndPoint - Domicilios
export async function fetchDomicilios(idCartera, idCuenta) {
  try {
    const response = await servicio.get(
      `/ejecutivo/domicilios/${idCartera}/${idCuenta}`
    );
    const data = response.data;
    return data;
  } catch (error) {
    if (error.response) {
      const errorMessage =
        error.response.data?.mensaje || "Error al recibir los domicilios";
      throw new Error(errorMessage);
    } else {
      throw new Error("Error en la respuesta del endpoint de domicilios.");
    }
  }
}
//EndPoint - Domicilios
