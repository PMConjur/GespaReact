...
namespace NoriAPI.Services
{
	...
	public class SearchService : ISearchService
	{
		...
		public async Task<(NewAddressResult, string, bool)> SaveNewAddress(NewAddressRequest newAddressData)
		{
			...
			var codigosPostales = await _searchRepository.SearchCodigosPostalesByCode(newAddressData.IdCodigoPostal);
			...
			var newAddress = new NewAddress
			{
				...
				IdCódigoPostal = bForaneo ? 0 : idCodigoPostal.Value, // Si es foráneo, asignamos 0
				Colonia = newAddressData.Colonia,
				IdClase = newAddressData.IdClase,
				Municipio = bForaneo && !string.IsNullOrWhiteSpace(newAddressData.Municipio) ? null : newAddressData.Municipio.Trim(),
				Estado = bForaneo && !string.IsNullOrWhiteSpace(newAddressData.Estado) ? null : newAddressData.Estado.Trim()
			};
